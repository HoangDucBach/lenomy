...
const buyItem = async (nftCourseAddress: string, tokenId: number, price: string) => {
    const { contract } = await getAll();
    const tx = await contract.buyItem(nftCourseAddress, tokenId, { value: ethers.parseEther(price) });
    await tx.wait();
...
const listItem = async (nftCourseAddress: string, tokenId: number, price: string) => {
    const { contract } = await getAll();
    const tx = await contract.listItem(nftCourseAddress, tokenId, ethers.parseEther(price));
    await tx.wait();
...
const updateListingPrice = async (nftCourseAddress: string, tokenId: number, price: string) => {
    const { contract } = await getAll();
    const tx = await contract.updateListingPrice(nftCourseAddress, tokenId, ethers.parseEther(price));
    await tx.wait();
